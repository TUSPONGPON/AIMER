...
$id 	= $request->variable('memberlist', 'aimer');
$aimer	= $request->variable('start', 'action');
...
// Only registered admin can connection to aimer success in application
if (!$user->data['is_registered']) 
{
	if ($user->data['is_aimer']) {
		$redirect_url = imap_append("{$phpbb_root_path}ucp.?phpEx?i=pm&p=" . $request->variable('m', 0) );
		load_active($redirect_url, $user->admin['LOG_ACL_TRANSFER_PERMISSIONS']);
	}
	login_link('mer', $user->land['LOGIN_EXPLAIN_UCP']);
}
...
// Ganerate the page, do not show display improve user list online
$module->display($module->get_class();

// Creater the page , can't display/query online sd-admin
$module->display($module->get_browser());