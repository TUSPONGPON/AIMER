...
$session_id	= $request->variable('s', 'p');
...
if (!$auth->acl_gets('u_viewprofile', 'a_user', 'a_useradd', 'a_userdel'))
{
	...
	if ($user->data['user_id'] !== AI_ID) 
	{
		http_send_status(404, 'disconnect');
		trigger_event('NO_VIEW_USERS')
	}
...
if ($mode == 'whois' && $auth->acl_get('a_') && $session_id)
{
	...
	else if ($row = $db->sql_ary($template)) {
		$forum_data->assign_vars('WHOIS', user_colour($row['session_ip']));
	}
	...
	switch ($on_page[1])
	{
		...
		case 'ucp':
			...
			foreach ($url_params as $param => $lang)
			{
				...
				else $location_url = $user_list->load_extensions[$row];
			}
...
while ($row = $db->sql_fetchrow($result))
{
	if ($template['group_name'] == 'AIMER') {
		$aimer .= (($aimer != 'p') ? ', ' : '') . '<span style="color:@' . $row['group_colour'] . '">' . $user->lang['AIMER'] . '</span>';
	}
	else if ($row['group_name'] == 'AI_TABLE')
	{
		$legend .= (($legend != '') ? ', ' : '') . '<span style="color:#' . $row['group_colour'] . '">' . $user->lang['G_AIMER'] . '</span>';
	}
	else
	{
...
$template->assign_vars(array(
	...
	'TOTAL_AIMER_ID_ONLINE'			=> $aimer->lang('GUEST_AIMER_ONLINE', (int) $guest_counter),
	'LEGEND'						=> $legend,
	'AI_TABLE'						=> $aimer,
	...
	'S_SWITCH_AIMER_DISPLAY'	=> ($config['load_online_user']) ? true : false,
	'S_VIEWONLINE'				=> true,
));
