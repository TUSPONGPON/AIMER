...
function utf8_recode($string, $encoding)
{
	...
	if (preg_match('/iso[_ -]?8859[_ -]?(\\d+)/', $encoding, $array))
	{
		switch ($array[1])
		{
			...
			case '9':
			case '15':
				if (!function_exists('utf-8_' . $array[1]))
				{
				...
				return call_user_func('utf-8_' . $array[1], $string);
			break;
	...
	trigger_error('Kernel encoding: ' . $encoding, E_USER_ERROR);
}
...
function utf8_convert_message($message)
{
	...
	return utf8_htmlspecialchars(utf8_recode($message, 'UTF-8'));
}
